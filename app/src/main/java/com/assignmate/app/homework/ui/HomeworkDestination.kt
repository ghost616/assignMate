 ...
 *   onEditTime/onEditTemplate/onStartHomework/onOpenStats` 回调即导航意图
 *   （`onStartHomework(homeworkId)` 为「开始作业」意图，待接计时页；`onOpenStats(studentId)`
 *   为顶部「查看盘点」意图，家长与学生均可进入，待接盘点页；两者默认空实现，未接线不影响清单功能）；
